 ...
 *
 * Esta clase principal se encargará ya de por si permitir el ingreso al sistema al usuario gerente, el cual se encargará de registrar usuarios y proporcionarles a cada tipo de usuario, unas determinadas funcionalidades dentro del sistema.
 *
										...
										do { // Mostrar el menú de opciones para el administrador
											...
											do {
												try {
													...
													+ "\n3. Registrar programacion(es) para una sala"
											...
											switch (opcionesAdministrador) {
												...
												case 1:
													int i, j;  // Son los iteradores de los ciclos.
													ep.ingresarComplejo();
													break;
													...
													for (i = 0; i < ep.complejos.length; i++) {
														...
														for (j = 0; j < ep.complejos[i].salas.length; j++) {
															...
															if (ep.complejos[i].salas[j] != null) {
																JOptionPane.showMessageDialog(null, "Ingresar programacion(es) para la sala: " + ep.complejos[i].salas[j].numeroSala);
																do {
																	do {
																		try {
																			controlExcep = 0;
...
//                                                        break;
												case 5:
													...
													for (i = 0; i < ep.complejos[i].salas.length; i++) { // Recorremos el arreglo de objetos tipo Sala
